...
module.exports = class Controller {
  ...
  getRepository() {
    const editorPath = lumine.workspace.getActiveTextEditor()?.getPath();
    const repository =
      (editorPath && lumine.repositories.getForPath(editorPath)) ||
      lumine.repositories.getActiveRepository() ||
  ...
  async runOperation(repository, operationName, callback, successMessage, fallback) {
    const operations = this.getOperations(repository, operationName, {
      silent: typeof fallback === "function",
    });
    if (operations) await callback(operations);
    else if (fallback) await fallback();
    else return false;
    if (successMessage) lumine.notifications.addSuccess(successMessage);
    return true;
  }

  async runRawWrite(repository, args) {
    await this.runRaw(repository, args);
    await Promise.all([
  ...
  async openChangedFiles() {
    ...
    await Promise.all(
      paths.map((filePath) => lumine.workspace.open(filePath, { activatePane: false })),
    );
  ...
  async restoreCurrentFile() {
    ...
    const answer = await lumine.workspace.confirmForPaneItem(editor, {
  ...
  async isProtected(repository, kind) {
    const enabled = lumine.config.get(
    ...
    const protectedBranches = lumine.config.get("git-command.protectedBranches") || [];
      ...
      onConfirm: async (item) =>
        this.runOperation(
          ...
          operationName === "rebase"
            ? () => this.runRawWrite(repository, ["rebase", item.branch])
            : undefined,
        ),
    });
    const refs = await repository.ensureRefsSnapshot();
    ...
    await this.modals.showSelection({
      ...
      onConfirm: (item) =>
        this.runOperation(
          ...
          () => this.runRawWrite(repository, ["cherry-pick", item.sha]),
        ),
    });
    const page = await repository.getCommits({
      revision: "--all",
  ...
  async pull() {
    ...
    await this.runOperation(
      ...
      "pull",
      (operations) =>
        operations.pull(null, null, { rebase: lumine.config.get("git-command.pullRebase") }),
  ...
  async push({ crumb } = {}) {
    ...
    if (!branch) {
      lumine.notifications.addInfo("The repository is not on a local branch.");
      return;
    }
      ...
      onConfirm: (message) => {
        ...
        return this.runOperation(
          ...
          () => {
            ...
            return this.runRawWrite(repository, args);
          },
        );
      },
    });
        ...
        await this.modals.showSecondarySelection({
          ...
          onConfirm: (action) =>
            this.runOperation(
              ...
              () =>
                this.runRawWrite(repository, [
                  "stash",
                  action.action.replace("stash", "").toLowerCase(),
                  stash.reference,
                ]),
            ),
        });
  ...
  run({ crumb } = {}) {
    ...
    this.modals.showInput({
      ...
      onConfirm: async (input, dialog) => {
        ...
        const result = await lumine.repositories.executeGit(args, repository.getWorkingDirectory());
