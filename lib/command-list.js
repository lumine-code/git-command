...
const ACTION_GROUPS = {
  ...
  branches: ["checkout", "new-branch", "merge", "rebase"],
  remotes: ["fetch", "fetch-all", "pull", "push"],
  stashes: ["stash", "manage-stashes"],
  advanced: ["run"],
};

const actionById = new Map(ACTIONS.map((item) => [item.action, item]));
const ACTION_SECTIONS = Object.entries(ACTION_GROUPS).map(([id, actionIds]) => ({
  id,
  items: actionIds.map((actionId) => actionById.get(actionId)),
}));
...
class CommandList {
  constructor(controller) {
    ...
    this.selectList = lumine.workspace.buildSelectList({
      ...
      commands: {
        "git-command:perform-and-close": {
          description: "Run the selected Git action.",
          didDispatch: (event) => this.perform(event.detail.item),
        },
        "git-command:perform-and-push": {
          description: "Continue the selected Git action in another modal step.",
          didDispatch: (event) => this.perform(event.detail.item),
        },
      },
      actions: [
        {
          command: "git-command:perform-and-close",
          context: "item",
          when: ({ item }) => !item.modal,
          primary: true,
          disposition: "close",
          dispatch: "local",
        },
        {
          command: "git-command:perform-and-push",
          context: "item",
          when: ({ item }) => Boolean(item.modal),
          primary: true,
          disposition: "push",
          dispatch: "local",
        },
  ...
  perform(item) {
    return this.controller.perform(item.action, { crumb: item.label });
  }
